...
d=int(input('Enter choice for data: 1)NLS 2)LS 3)Image 4)PCA\n'))
...
for i in range(0,Nc):
    ...
    Xx=pd.read_csv(path+"Class "+str(i+1)+"_train.txt",header = None, delimiter=',', usecols=(0, 1))
    Xx=np.array(Xx)
...
plt.title('Decision region plot for train data')
...
for i in range(0,Nc):
    ...
    temp=pd.read_csv(path+"Class "+str(i+1)+"_test.txt",header = None, delimiter=',', usecols=(0, 1))
    temp=np.array(temp)
    m,n=temp.shape
    ytemp=i*np.ones((m,1))
...
plt.title('Decision region plot Non Linearly Seperable data')
plt.show()
...
ax.set_ylabel('y')
ax.set_zlabel('PDF')
ax.set_title('Surface plot of Gaussian 2D KDE for PCA Data')
